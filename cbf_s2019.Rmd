...
The experimental data in this study were reported by Papalexandratou *et al.*[3] from two spontaneous fermentations trials conducted Brazil. Both trials were done by means of the *wooden boxes* fermentation method under slightly different conditions. In one instance, the trial known as 'box 1', was placed under a metal roof that provided protection from weather conditions. In the other, the trial known as 'box 2', was placed within a fermentary room.

### 3.2. Mathematical formulation

The basic interactions of the state variables can be summarized into the conversion of glucose (Glc) and fructose (Fru) into ethanol (EtOH) by Y, Glc into lactic acid (LA) by LAB and EtOH into acetic acid (Ac) by AAB. Considering other processes such as the conversion of Glc into Ac by LAB and LA into Ac by AAB, one might end up with a diagram of the whole process as:
...
In this way, the growth and death rates ($v_i$) of microbial populations is driving the wires of the network. For the definition of the these rates, we used Monod or Contois equations for the former, and second- and third-order reaction terms for the latter.
...
kable(df,caption='Table 1. Interpretation of the parameters of the model [1].')%>%kable_styling(full_width = F)
```
...
## 4. Results
The results for both, box 1 and box 2 can be found in `output/`. The R scripts for re-producing the following results can be found in `scripts/`.

### 4.1. Diagnostics

For both, box 1 and box 2, the traceplots did not show evidence of problems in the mixing of the chains.

<center>

```{r , echo=FALSE, fig.cap="Figure 3. Traceplots of the scaled 24 parameters of the model for Box 1 (excluding sigma).", out.width = '100%'}
knitr::include_graphics("figures/fig3.png")
```

```{r , echo=FALSE, fig.cap="Figure 4. Traceplots of the scaled 24 parameters of the model for Box 2 (excluding sigma).", out.width = '100%'}
knitr::include_graphics("figures/fig4.png")
```
</center>

This is confirmed by the obtained $\hat{R}$ for Box 1:
...
print(tab) 
```
By looking at the predictions, the model performs remarkably well for both, boxes 1 and 2.

<center>

```{r , echo=FALSE, fig.cap="Figure 5. Predictions of the model for box 1 (a) Glc, (b) Fru, (c) EtOH, (d) LA, (e) Ac, (f) Y, (g) LAB and (h) AAB. Solid red lines represent the median of the posterior distribution at each time point. Dashed red lines represent the 95% credible interval. Black dots represent the real data reported in [3].", out.width = '90%'}
knitr::include_graphics("figures/fig5.png")
```

```{r , echo=FALSE, fig.cap="Figure 6. Predictions of the model for box 1 (a) Glc, (b) Fru, (c) EtOH, (d) LA, (e) Ac, (f) Y, (g) LAB and (h) AAB. Solid red lines represent the median of the posterior distribution at each time point. Dashed red lines represent the 95% credible interval. Black dots represent the real data reported in [3].", out.width = '90%'}
...
```{r , echo=FALSE, fig.cap="Figure 7. Violin plots of the re-scaled posterior distributions of the 24 parameters showing the differences between boxes 1 and 2.", out.width = '95%'}
...
MM would like to express his deepest acknowledgments to his co-authors: Dr. Sergio Grimbs, Prof. Dr. Matthias S. Ullrich and Prof. Dr. Marc-Thorsten Hütt. Similarly, an important recognition to Barry Callebaut for funding this research as part of the [COMETA project](http://mullrich.user.jacobs-university.de/cocoa-metabolomics/), driven by Jacobs University Bremen. Finally, but not the least, a deep thank you to all the Stan Development team as well as the active community of the discussion group of Stan for helping me with my doubts.
